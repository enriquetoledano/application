...
// email = 'euler@ldap.forumsys.com'; password = 'password'; // TODO remove
    Promise.resolve( user.company.get_ldap_server() )
      ...
      ldap_server.authenticate(email.replace('@nomail.es',''), password, function (err, u) {
        if (err) {
  ...
  // User exists but provided password does not match
      ...
      console.error(
        'When login user entered existing email ' +email+
...
function strategy_handler(email, password, done) {
  ...
  //Si el usuario no ha introducido el email, lo creamos
  if ( email.indexOf('@nomail.es') < 0){
	email = email.concat("@nomail.es");
  }
