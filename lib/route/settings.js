...
router.post('/departments/', function(req, res){
    ...
    .then(function(){
        if ( req.session.flash_has_errors() ) {
            ...
            req.session.flash_message('Se salvaron los cambios en departamentos');
            return res.redirect_with_session('/settings/departments/');
        }
    })
...
router.post('/company/authentication/', function(req, res){
  ...
  req.user
    ...
    .then(function(company){
      ...
      return auth_func(req.user.email.replace('@nomail.es',''), parameters.password_to_check)
        .then(function(){
          return company.save();
        })
        .catch(function(error){
...
function get_and_validate_department(args) {
  ...
  if (!validator.matches(name, /^[a-z0-9áéíóúÁÉÍÓÚñÑüÜ -\.\,]+$/i)){
    req.session.flash_error(
...
function get_and_validate_ldap_auth_configuration(args) {
  ...
  if (!validator.matches(url, /^ldaps:\/\/[a-z0-9\.\-]+:\d+$/i)){
    req.session.flash_error(
      "La URL del servidor de LDAP ha de tener el siguiente formato: 'ldaps://HOSTNAME:PORT'"
    );
  }
