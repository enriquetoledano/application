...
function leave_request_action(args) {
    ...
    .then(function(processed_leave){
      req.session.flash_message('La petición de '+processed_leave.user.full_name()
          +' fue '+current_action+'ed');
...
router.post(
  ...
  function(req, res){
    ...
    .then(function(processed_leave){
      req.session.flash_message('Ha solicitado que su petición de día libre sea revocada, y su supervisor tiene que aprobarlo.');

      return res.redirect_with_session('../');
    })
    .catch(function(error){
      console.error('Ocurrió un error cuando se intentaba revocar una petición de día libre '
          +request_id+' del usuario '+req.user.id+' Error: '+error
      );
      req.session.flash_error('Fallo al revocar una petición de revocación de día libre');
      return res.redirect_with_session('../');
    });
  }
);
