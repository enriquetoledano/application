...
router.post('/edit/:user_id/', function(req, res){
  ...
  // Extra step: in case when employee had "end date" populated and now it is going
  ...
  .then(function(){
    ...
    employee.updateAttributes(new_user_attributes).then(function(){
      req.session.flash_message(
        'Los detalles de '+employee.full_name()+' se actualizaron'
      );
      return res.redirect_with_session('.');
    });
  })
...
router.post('/delete/:user_id/', function(req, res){
    ...
    .then(function(result){
        req.session.flash_message(
            'Los registros del empleado se eliminaron del sistema'
        );
        return res.redirect_with_session('../..');
    })
    .catch(function(error){
...
function get_and_validate_user_parameters(args) {
    ...
    if (!validator.matches(name, /^[a-záéíóúÁÉÍÓÚñÑüÜ \-\.]+$/i)){
        req.session.flash_error(
            'El nuevo nombre de usuario sólo debe contener caracteres alfabéticos'
        );
    }

    if (!validator.matches(lastname, /^[a-z0-9áéíóúÁÉÍÓÚñÑüÜ \-\.\,]+$/i)){
        req.session.flash_error(
            'Los nuevos apellidos de usuario sólo deben contener caracteres alfabéticos'
        );
    }
    ...
    // Normalize email as we oprate only with lower case letters in emails
