...
module.exports = function(passport) {
  ...
  router.post('/login', function(req, res, next) {
    passport.authenticate('local', function(err, user, info) {
      ...
      if (!user) {
        req.session.flash_error('Credenciales incorrectas');
      ...
      req.logIn(user, function(err) {
        ...
        req.session.flash_message('¡Bienvenido de nuevo '+user.name+'!');
  ...
  router.post('/register', function(req, res){
      ...
      if (!email){
          req.session.flash_error('No se proporcionó el Email');
      } else if ( ! email_validation_re.test( email )) {
          req.session.flash_error('La dirección de Email no es válida');
      }

      var name = req.param('name');
      if (!name){
          req.session.flash_error('No se especificó el nombre');
      }

      var lastname = req.param('lastname');
      if (!lastname) {
          req.session.flash_error('No se especificaron los apellidos');
      }
      ...
      if (!validator.matches(company_name, /^[a-z0-9 \.\,]+$/i)){
        req.session.flash_error('El nombre de la compañía debería contener sólo letras y números');
      }

      var password = req.param('password');
      if (!password) {
          req.session.flash_error('La contraseña no puede estar en blanco');
      } else if ( password !== req.param('password_confirmed') ) {
          req.session.flash_error('La confirmación de contraseña no coincide con la inicial');
      }

      var country_code = req.param('country');
      if (! validator.matches(country_code, /^[a-z]{2}/i) ){
          req.session.flash_error('Código de pais incorrecto');
      }
